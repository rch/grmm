 ...
 * @version $Id: RandomGraphs.java,v 1.9 2006/05/08 19:07:38 casutton Exp $
 */
public class RandomGraphs {
  ...
  public static FactorGraph createRandomChain (org.zndx.oss.mallet.base.util.Random r, int length)
  {
    Variable[] vars = new Variable[length];
    for (int i = 0; i < length; i++)
      vars[i] = new Variable (2);
  ...
  {
    UndirectedModel mdl = new UndirectedGrid (size, size, 2);
    for (Iterator it = mdl.getEdgeSet ().iterator (); it.hasNext();) {
      VarSet vars = (VarSet) it.next ();
