...
//import org.zndx.oss.mallet.base.pipe.SerialPipe;
...
import org.zndx.oss.mallet.base.types.ArraySequence;
import org.zndx.oss.mallet.base.types.SequencePair;
import org.zndx.oss.mallet.base.types.SequencePairAlignment;
...
import org.zndx.oss.mallet.base.types.DenseVector;
...
import org.zndx.oss.mallet.base.util.search.*;
...
//							destinationNode.alpha = INFINITE_COST;
	...
	public ViterbiPath viterbiPath (Sequence inputSequence, Sequence outputSequence, boolean saveLattice)
	{
		// xxx We don't do epsilon transitions for now
		assert (outputSequence == null
						|| inputSequence.size() == outputSequence.size());
		return new ViterbiPath (inputSequence, outputSequence, saveLattice);
	}
...
//						System.out.println("cost = " + iter.getCost() + " (" + ip + " " + i +")");
			...
			for(int i=1; i<numStates; i++){
				...
				for(int j=0; j<i; j++){
					if (nodes[latticeLength-1][i] == null)
						getViterbiNode(nodes, latticeLength-1, i);
