...
import org.zndx.oss.mallet.base.types.Alphabet;
import org.zndx.oss.mallet.base.types.FeatureSequence;
...
public class FeatureVector extends SparseVector implements Serializable
{
  ...
  public static FeatureVector newFeatureVector (FeatureVector fv, Alphabet newVocab, FeatureSelection fs)
  {
    ...
    if (fv.indices == null || fv.values == null)
      throw new UnsupportedOperationException("Not yet implemented for dense or binary feature vectors.");
    ...
    int[] indices2 = new int[fvNumLocations];
    double[] values2 = new double[indices2.length];
    int size = 0;
    for (int i = 0; i < fvNumLocations; i++) {
      if (fs.contains(fv.indices[i])) {
        try{
          indices2[size] = newVocab.lookupIndex(fv.dictionary.lookupObject(fv.indices[i]), true);
        } catch (Exception e) {
          System.out.println (e.toString());
        }
        values2[size] = fv.values[i];
        size++;
      }
    }
    return new FeatureVector (newVocab, indices2, values2, size, size, true, true, false);
  }
