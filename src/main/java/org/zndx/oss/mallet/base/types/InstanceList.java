...
import org.zndx.oss.mallet.base.pipe.PipeOutputAccumulator;
import org.zndx.oss.mallet.base.pipe.SerialPipes;
import org.zndx.oss.mallet.base.pipe.TokenSequence2FeatureSequence;
import org.zndx.oss.mallet.base.pipe.FeatureSequence2FeatureVector;
import org.zndx.oss.mallet.base.pipe.Target2Label;
import org.zndx.oss.mallet.base.pipe.iterator.PipeInputIterator;
import org.zndx.oss.mallet.base.pipe.iterator.RandomTokenSequenceIterator;
import org.zndx.oss.mallet.base.util.MalletLogger;
import org.zndx.oss.mallet.base.util.PropertyList;
import org.zndx.oss.mallet.base.util.Random;
import org.zndx.oss.mallet.base.util.DoubleList;
...
public class InstanceList implements Serializable, PipeOutputAccumulator
{
  ...
  // CPAL - extracts all the instances for which there are non-empty features
  public int[] subListFlagNotEmpty(int start, int end) {
    InstanceList other = new InstanceList (getPipe ());
    Instance tmpI;
    AugmentableFeatureVector afv;
    ...
    for (int i = start; i < end; i++) {
        tmpI = getInstance(i);
        afv = (AugmentableFeatureVector)tmpI.getData();
        afv.numLocations();
        if(afv.numLocations() > 1) {
  ...
  public InstanceList subListFlag (int start, int end, int[] flag)
  {
    InstanceList other = new InstanceList (getPipe ());
    int j=0;
...
//                      System.out.println(i + ": " +  index +": " + inst.getTarget().toString() 
  ...
  // CPAL - split at a given point in the instance list
  ...
  // I'm not sure these methods best belong here. -AKM Jan 2006
  public void hideSomeLabels (double proportionToHide, Random r)
  {
    for (int i = 0; i < instances.size(); i++) {
