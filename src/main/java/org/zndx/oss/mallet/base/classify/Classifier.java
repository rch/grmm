...
import org.zndx.oss.mallet.base.util.ArrayListUtils;
...
public abstract class Classifier
{
  ...
  protected Classifier()
  {
    ...
    // CPAL - added this to get the Average position of the correct target
    public double getAvgPosAc(InstanceList ilist)
    {
        return getAvgPosAc(classify(ilist));
    }

    public double getAvgPosAc(ArrayList classifications)
    {
        double rsum = 0;
        Labeling tmpL;
        Classification tmpC;
        ...
        for(int i=0;i< classifications.size(); i++) {
            tmpC = (Classification)classifications.get(i);
            tmpI = tmpC.getInstance();
            tmpL = ((Labeling)(tmpC).getLabeling());

            tmpLbl = (Label)tmpI.getTarget();
            tmpInt = tmpL.getRank(tmpLbl);
            tmpLbl2 = tmpL.getLabelAtRank(0);

            System.out.println (tmpInt + "   " + tmpLbl.toString() + " Best was: " + tmpLbl2 );

            rsum = rsum + (double)tmpInt;
        }

        return rsum/classifications.size();
