 ...
 * @version $Id: LatticeViewer.java,v 1.12 2005/03/05 05:26:21 casutton Exp $
 */
public class LatticeViewer {
  ...
  private static void outputLatticeRows (PrintWriter out, Transducer.ViterbiPath lattice, int start, int end)
  {
    ...
    for (int k = 0; k < max; k++) {
      ...
      for (int ip = start; ip < end; ip++) {
          Transducer.State state = lattice.getStateAtRank (ip+1, k);
        if (state.getName().equals (lattice.output ().get(ip))) {
  ...
  private static void outputTransitionCosts (PrintWriter out, ExtorInfo info, int start, int end)
  {
    ...
    for (int ip = start; ip < end; ip++) {
      ...
      Transducer.State from = ((CRF4) ducer).getState (info.bestStates.get (ip - 1).toString ());
    ...
    for (int ip = start; ip < end; ip++) {
      ...
      if (!seqMatches (info.predicted, info.target, ip) || !seqMatches (info.predicted, info.target, ip - 1)) {
        Transducer.State from = ((CRF4) ducer).getState (info.target.get (ip - 1).toString ());
        if (from == null) {
  ...
  private static void outputLatticeRows (PrintWriter out, Transducer.Lattice lattice, int start, int end)
  {
    DecimalFormat f = new DecimalFormat ("0.##");
    Transducer ducer = lattice.getTransducer ();
    ...
    for (int k = 0; k < ducer.numStates(); k++) {
      ...
      for (int ip = start; ip < end; ip++) {
          out.print ("<td>"+f.format (lattice.getGammaCost (ip+1, state))+"</td>");
      }
      out.println ("</tr>");
    }
  }
  ...
  private static class ExtorInfo {
    ...
    Transducer.ViterbiPath lattice;
    Sequence bestStates;
...
//    TokenSequence input = (TokenSequence) c2.getData ();
    ...
    if (showLattice == true) {
      CRF4 crf = extor.getCrf();
      // xxx perhaps the next two lines could be a transducer method???
      Instance carrier = new Instance (input, null, null, null, extor.getFeaturePipe ());
      info.fvs = (FeatureVectorSequence) carrier.getData ();
      info.lattice = crf.viterbiPath ((Sequence) carrier.getData(), true);
      info.bestStates = info.lattice.getBestStates ();
    }
