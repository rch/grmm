...
package org.zndx.oss.mallet.grmm.learning;

import org.zndx.oss.mallet.base.maximize.LimitedMemoryBFGS;
import org.zndx.oss.mallet.base.maximize.Maximizable;
import org.zndx.oss.mallet.base.maximize.Maximizer;
import org.zndx.oss.mallet.base.types.*;
import org.zndx.oss.mallet.base.util.MalletLogger;
import org.zndx.oss.mallet.base.util.Timing;
import org.zndx.oss.mallet.grmm.learning.ACRF.MaximizableACRF;
import org.zndx.oss.mallet.grmm.learning.ACRF;
import org.zndx.oss.mallet.grmm.learning.ACRFEvaluator;
 ...
 * @version $Id: ACRFTrainer.java,v 1.3 2006/02/11 03:47:08 casutton Exp $
 */
public class ACRFTrainer {

  private static Logger logger = MalletLogger.getLogger (ACRFTrainer.class.getName ());
  ...
  public boolean train (ACRF acrf,
                        ...
                        int numIter)
  {
    Maximizable.ByGradient macrf = createMaximizable (acrf, trainingList);
    return train (acrf, trainingList, validationList, testSet,
            eval, numIter, macrf);
  }

  protected Maximizable.ByGradient createMaximizable (ACRF acrf, InstanceList trainingList)
  {
...
*/
                                   ...
                                   int numIter)
  {
    ...
    for (int i = 0; i < SIZE.length; i++) {
      ...
      Maximizable.ByGradient subset_macrf = createMaximizable (acrf, subset);
...
//		Maximizer.ByGradient maximizer = new GradientDescent ();
    ...
    for (int i = 0; i < numIter; i++) {
      ...
      logger.info ("ACRF trainer iteration " + i + " at time " + (etime - stime));

      try {
        converged = maximizer.maximize (macrf, 1);
        converged |= callEvaluator (acrf, trainingList, validationList, testSet, i, eval);
        ...
        // give up.
        if (resetOnError && (maximizer instanceof LimitedMemoryBFGS)) {
          logger.warning ("Exception in iteration " + i + ":" + e + "\n  Resetting LBFGs and trying again...");
   ...
   */
  private boolean callEvaluator (ACRF acrf, InstanceList trainingList, InstanceList validationList,
                                 InstanceList testSet, int iter, ACRFEvaluator eval)
  {
    ...
    if (!eval.evaluate (acrf, iter, trainingList, validationList, testSet)) {
...
//			System.out.println ("\n\nInstance");
        LabelsSequence target = (LabelsSequence) inst.getTarget ();
...
//		results.printConfusion ();
  ...
  public static class TestResults {
    ...
    {
      Labels lbls = ((LabelsSequence) inst.getTarget ()).getLabels (0);
      factors = new TIntArrayList [lbls.size ()];
      for (int i = 0; i < lbls.size (); i++) {
        Label l = lbls.get (i);
        LabelAlphabet dict = l.getLabelAlphabet ();
